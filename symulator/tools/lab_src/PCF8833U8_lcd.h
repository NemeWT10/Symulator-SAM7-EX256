...
#define TRANSPARENT2 -1
typedef struct {
    const unsigned char *data; // 12bpp: 2 pix = 3 bajty
    int w;                     // szerokosc (CASET / y)
    int h;                     // wysokosc  (PASET / x)
    int x0;                    // offset na LCD (x)
    int y0;                    // offset na LCD (y)
} LCD_BMP12;

void LCDSetTransparentBackground(const unsigned char *bmpArr, int w, int h, int x0, int y0);

unsigned int BmpGetPixel12(const LCD_BMP12 *img, int screenX, int screenY, unsigned int fallback);
...
void LCDClearXY(int x, int y, int size, int w, int
cColor);
int LCDPutStr2(int x, int y, int Size, int fColor, int
bColor, char *pString);
void LCDDrawBmp12(const unsigned char *bmpArr, int w, int h, int x0, int y0);
#ifdef __cplusplus
}
#endif